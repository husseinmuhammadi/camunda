package com.camunda.bpmn.engine;
...
class BpmnParserTest {
    ...
    @BeforeEach
    void setUp() {
        modelInstance = Bpmn.readModelFromStream(this.getClass().getClassLoader().getResourceAsStream("mock-data/invoice-approval.xml"));
    }

    @Test
    void parseInvoice() throws Exception {
        Assertions.assertNotNull(modelInstance);

        Collection<FlowNode> nodes = modelInstance.getModelElementsByType(FlowNode.class);

        nodes.stream().map(FlowElement::getId).forEach(System.out::println);
    }
